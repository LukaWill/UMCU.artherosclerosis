...
#plaque types / clinical info
plaque_types <- "Seurat_clusters.txt"
clinical_info_file <- "clinical_data_good_selection.txt"

#directory path where the networks are 
directory_path <- "~/Graduation/Rstudio/Test/Networks"

#selecting clusters withing network, this only when the correlation withing a network seems to separate into two clusters
cluster_network <- F
...
#Select column name of feature and input file, either clinical_info or cluster
#features <- clinical_info %>% select('study_number','symptoms_inclusion')
features <- cluster
  
#name of the module score file that includes the clusters 
module_score_file <- "module_score_networks_ALL_plaque_cluster.csv"
...
#network_names_N <- c("network28", "network82", "network177", "network39","network204", "network74", "network131", "network212", "network114", "network171", "network191", "network185", "network156", "network170", "network44", "network76", "network168", "network31", "network123", "network91", "network97", "network119")
...
result_no_overlap <- "network_genes_no_overlap_percentage_ALL_plaque_cluster.txt"

#violin plot module score
violin_plot_module <- "~/Graduation/Rstudio/test/violin_plot_module_score_ALL_plaque_cluster.pdf"

#Connectivity

heatmap_cor = "~/Graduation/Rstudio/Test/all_correlation_ALL_plaque_cluster.pdf"
dendogram_cluster_genes <- "cluster_genes_correlation_ALL_plaque_cluster.csv"

directory <- "~/Graduation/Rstudio/Test/Networks"

average_correlation_file <- "average_correlation_total_ALL_plaque_cluster.RData"
#connectivity plots
connectivity_plot <- "~/Graduation/Rstudio/Test/connectivity_grid_ALL_plaque_cluster.pdf"
